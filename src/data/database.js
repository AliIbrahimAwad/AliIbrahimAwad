...
class CrmDatabase extends BaseCrmDatabase {
  ...
  updateApiLeadStatus(id, status, actor = null, options = {}) {
    ...
    this.save();
    this.refreshEmailIntakeStateForLead(id);
  ...
  getEmailIntakeItemByExternalId(externalId, dealershipId = getDefaultDealershipId()) {
  ...
  getEmailIntakeItem(id, user = null) {
    const dealershipId = this.currentDealershipId(user);
    const row = this.get(
      `
        ${this.emailIntakeSelectSql()}
  ...
  getEmailIntakeSummary(user = null) {
    const dealershipId = this.currentDealershipId(user);
    const rows = this.all(
      `
  ...
  listEmailIntakeItems(filters = {}, user = null) {
    const dealershipId = this.currentDealershipId(user);
    ...
    const countRow = this.get(`SELECT COUNT(*) AS count FROM email_intake_items WHERE ${clauses.join(" AND ")}`, params);
    const rows = this.all(
      `
        ${this.emailIntakeSelectSql()}
        WHERE ${clauses.join(" AND ")}
    ...
    return {
      ...
      summary: this.getEmailIntakeSummary(user),
    };
  }

  createEmailIntakeItem(input = {}, user = null) {
    const now = new Date().toISOString();
    const dealershipId =
      parsePositiveInteger(input.dealership_id) || (user ? this.currentDealershipId(user) : getDefaultDealershipId());
  ...
  refreshEmailIntakeStateForLead(leadId) {
    const lead = this.getApiLead(leadId);
  ...
  assignEmailIntakeItem(id, assignedTo, user = null) {
    const item = this.getEmailIntakeItem(id, user);
    ...
    this.assignLead(leadId, assignedTo, user);
    this.execute(
      `
        UPDATE email_intake_items
    ...
    this.save();
    this.refreshEmailIntakeStateForLead(leadId);

    return this.getEmailIntakeItem(id, user);
  }

  resolveEmailIntakeItem(id, user = null) {
    const item = this.getEmailIntakeItem(id, user);
  ...
  convertEmailIntakeItemToLead(id, input = {}, user = null) {
    const item = this.getEmailIntakeItem(id, user);
    if (item.classification !== "other") {
    ...
    const assignedTo = parsePositiveInteger(input.assigned_to);
    if (assignedTo) {
      this.assignLead(leadId, assignedTo, user);
    }
    ...
    return {
      item: this.getEmailIntakeItem(id, user),
      lead: this.getApiLeadWithActivities(leadId, user),
    };
  }
