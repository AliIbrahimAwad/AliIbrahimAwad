...
function ManagerPages(props) {
  ...
  if (activePage === "lead-detail") {
    return <LeadWorkspaceSection lead={selectedLead} detailLoading={false} buildVehicleLabel={buildVehicleLabel} getLeadChipTone={getLeadChipTone} getLeadNextAction={getLeadNextAction} onOpenSms={() => onOpenLead(selectedLead?.id, { page: "lead-detail", drawer: true })} onOpenAssign={() => onOpenLead(selectedLead?.id, { page: "lead-detail", drawer: true })} onOpenAppointment={() => onOpenLead(selectedLead?.id, { page: "lead-detail", drawer: true })} />;
  }

  if (activePage === "comms") {
    return <div className="split-3"><section className="card"><div className="section-title"><h3>Conversations</h3><Chip tone="red">{visibleConversationFeed.length} active</Chip></div><div className="list">{visibleConversationFeed.map((item) => <button key={item.id} type="button" className="list-item button-reset" onClick={() => item.leadId && onOpenLead(item.leadId, { page: "comms", drawer: true })}><div className="row"><strong>{item.leadName || "Unknown lead"}</strong><Chip tone={getConversationChipTone(item)}>{item.type === "call" ? "Call" : "SMS"}</Chip></div><div className="tiny">{item.preview}</div><div className="tiny">{item.happenedAtLabel}</div></button>)}</div></section><section className="card"><div className="section-title"><h3>Customer context</h3><Chip tone="blue">Live CRM data</Chip></div>{selectedLead ? <div className="list"><div className="list-item"><strong>{selectedLead.customerName}</strong><div className="tiny">{selectedLead.phone} | {selectedLead.email}</div></div><div className="list-item"><strong>Target vehicle</strong><div className="tiny">{buildVehicleLabel(selectedLead)}</div></div><div className="list-item"><strong>Next move</strong><div className="tiny">{getLeadNextAction(selectedLead)}</div></div></div> : <EmptyState label="Open a lead conversation to see thread context here." />}</section><section className="card"><div className="section-title"><h3>Unknown inbox</h3><Chip tone="amber">{visibleUnmatchedItems.filter((item) => item.status === "new").length} new</Chip></div><div className="list">{visibleUnmatchedItems.slice(0, 6).map((item) => <button key={item.id} type="button" className="list-item button-reset" onClick={() => onSetPage("unmatched")}><div className="row"><strong>{item.phone}</strong><Chip tone={getUnmatchedChipTone(item)}>{item.type === "call" ? "Call" : "SMS"}</Chip></div><div className="tiny">{item.preview}</div><div className="tiny">{item.receivedAtLabel}</div></button>)}</div></section></div>;
  }
...
function RepPages({ activePage, selectedLead, myLeadLibrary, myConversationFeed, myAppointments, myUrgentLeads, myTasks, buildVehicleLabel, getLeadChipTone, getConversationChipTone, formatCurrencyCompact, onOpenLead }) {
  ...
  if (activePage === "rep-customer") {
    return <LeadWorkspaceSection lead={selectedLead} detailLoading={false} buildVehicleLabel={buildVehicleLabel} getLeadChipTone={getLeadChipTone} getLeadNextAction={() => ""} onOpenSms={() => onOpenLead(selectedLead?.id, { page: "rep-customer", drawer: true })} onOpenAssign={() => onOpenLead(selectedLead?.id, { page: "rep-customer", drawer: true })} onOpenAppointment={() => onOpenLead(selectedLead?.id, { page: "rep-customer", drawer: true })} />;
  }
